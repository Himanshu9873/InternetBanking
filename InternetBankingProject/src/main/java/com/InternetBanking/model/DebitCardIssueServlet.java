...
public class DebitCardIssueServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(accountNumber!=null && cardHolderName!="" && issueDate!="" && expiryDate!="" && balance!=null && pinNumber!=null) {
				PrintWriter writer = response.getWriter();
				writer.println("Welcome dear");
			}
			else {
				session.setAttribute("message", "OOPS!! Plese Fill all fields");
				response.sendRedirect("issueDebitCard.jsp");
				return ;
			}
